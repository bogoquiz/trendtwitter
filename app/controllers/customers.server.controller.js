	...
	Country.find(function(err, country){
          ...
          //client.get('/trends/place',{id: 23424757}, function(err, payload){  
            ...
            for (var i = 0 ; i < 4; i++) {
              ...
              if (x<links.length){  
                  trend = trend + ' http://trendmedia.herokuapp.com #FAIL' + ' http://youtu.be/' + links[32].videos.id.videoId;
                  }else{
                    trend = trend + ' http://youtu.be/' + links[32].videos.id.videoId;
              ...
              for (var i = 1 ; i < 2; i++) {
              	z = payload[0].trends[i].name + ' http://trendmedia.herokuapp.com #FAIL' + ' http://youtu.be/' + links[32].videos.id.videoId;
